...
class ChatAssistantProvider:
    ...
    """
    def __init__(self, model="deepseek/deepseek-chat", memory=None):
    ...
    def get_assistant(self):
        with _lock1:
            ...
            self._assistant = ChatAssistant(memory=self._memory)
            self._assistant.model_manager.change_model(self._model)
            return self._assistant

# シングルトン的に使う場合のグローバル関数
def get_chat_assistant(model="deepseek/deepseek-chat", memory=None):
    global _instance
    with _lock2:
        if _instance is None:
            _instance = ChatAssistantProvider(model=model, memory=memory)
